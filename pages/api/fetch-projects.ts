...
if (!GITHUB_TOKEN) {
  throw new Error('El GitHub token no está definido en el .env.local');
}
...
export default async function handler(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('Error al obtener los respositorios de GitHub');
    }
    ...
    res.status(500).json({ message: 'Error al obtener repos de github' });
  }
}
